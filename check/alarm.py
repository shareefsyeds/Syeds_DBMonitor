...
def check_alarm():
    alarm_time = now()
    checklog.logger.info('Initialize the alarm information table')
    ...
    for each_check in check_list:
        ...
        checklog.logger.info("Began to examine the alarm：{}" .format(alarm_name))
        select_sql = "select count(*) from {}".format(judge_table)
        select_res = mysql_query(select_sql)
        if select_res[0][0] == 0:
            checklog.logger.info("%s Not collected data" % alarm_name)
        else:
            ...
            if check_res == 0:
                checklog.logger.info("{}:Alarms check normal" % alarm_name)
            else:
